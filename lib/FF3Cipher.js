...
const power = (a,n) => n===0n ? 1n : a * power(a, n-1);
...
class FF3Cipher {
    ...
    encrypt(plaintext) {
		...
		// The above calculation fails for exponents greater than 20 Math.pow(10,20) which returns a float instead of an int
        // and may incorrectly round the result starting with exp 23.

		if (modU > Number.MAX_SAFE_INTEGER) {
          console.error("plaintext length is too long for MAX_SAFE_INTEGER");
		}
